...
| Animated GIF upload | `GifAgreement.java` | ✅ `protocol/gif.py` — hardware-verified. CORRECTION 2026-07-25 (vendor-app HCI capture): the `time_sign`/`ConvertTime` claim was wrong — the field is little-endian and the app's default key emits 5, not 10; fixed in `_convert_time_sign`. Only the no-time-signature branch (the one the client uses) was ever exercised, which is why the error stayed invisible |
