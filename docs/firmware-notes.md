...
| Screen-timeout family | No ack and no visible effect at all on the reference 32×32 (`device.set_screen_timeout` / `read_screen_timeout`). Likely a feature this specific panel doesn't implement, not a universal protocol dead-end — units and support are otherwise unknown pending a panel that *does* respond. | probe 2026-07-12 |
...
| Lazy display-state persistence | The panel commits its *displayed* mode to flash lazily, so content written and disconnected-from within seconds is silently lost, while config-class state (brightness, RTC, alarms, eco) commits durably on any connection. Two distinct timings govern this and must not be swapped — full account in [Protocol Notes § Persistence](protocol-notes.md#persistence-and-durability): surviving a **BLE reconnect** needs ~100–180 s of dwell (or a prior reconnect, ~10 s of which is already enough); surviving a **power cut** is the separate flash-commit question, settled 2026-07-29 at wall-clock 5 s < t ≤ 10.3 s from the write, independent of link state. Recorded here because persistence timing is exactly the kind of thing that could plausibly differ by firmware revision, and only one revision has been tested. | probes 2026-07-12, 07-17, 07-27/28, 07-29 |
