...
| Lazy display-state persistence | The panel commits its *displayed* mode to flash lazily, so content written and disconnected-from within seconds is silently lost, while config-class state (brightness, RTC, alarms, eco) commits durably on any connection. Documented in full in [Protocol Notes](protocol-notes.md#persistence-and-durability) — recorded here because persistence timing is exactly the kind of thing that could plausibly differ by firmware revision, and only one revision has been tested. The dwell threshold (somewhere between 8 s and 90 s) has not been bisected even on that one. | probes 2026-07-12, 07-17, 07-27/28 |
