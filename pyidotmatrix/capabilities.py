...
_ENTRIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        ">=2 of the client session. Brightness's class is UNKNOWN until `--no-reset brightness` "
        "runs (queued, docs/PROBE_PLAN.md P19). POINTER-NOT-PAYLOAD, HYPOTHESIS, NOT SETTLED: the "
        "shadow may kill only the current-mode POINTER while stored/flash payload survives "
        "underneath it. Consistent with all three of -- fullscreen colour, which has no stored "
        "payload at all, dying identically to the GIF; P10's interrupted-upload runs finding "
        "stored GIF content intact through interruptions (see gif.upload_file); and config-class "
        "writes committing normally on first connections. PREDICTION: after a shadowed GIF has "
        "died back to the clock, gif.activate_stored() on the POST-reconnect session restores it "
        "WITHOUT a re-upload. Confirm probe: `probe_p11_persistence.py shadow-recover`, queued. "
        "If it confirms, the recovery guidance for callers becomes RE-ACTIVATE, DO NOT "
        "RE-TRANSFER. CONSISTENT-WITH, from the older record (inferred topology, not proof): "
        ...
        "MITIGATION IS IMPLEMENTED IN THIS DRIVER, and the cheap one is now off the table: with "
        "`--preamble power` having died, the only known lift is a real throwaway "
        "connect/disconnect/reconnect at startup, which is a workaround for a mechanism nobody "
        "understands and should wait on the shadow-recover result. GLANCEOS "
    ...
    Capability(
        ...
        "(probes/probe_p19_g3_clock_styles.py; docs/PROBE_PLAN.md P19). "
        "RENDERING MODEL: STYLE_COLOR (3) colours the "
    ...
    Capability(
        ...
        "record. Its payload (a device/firmware descriptor?) is undecoded. CALLER CAVEAT, "
        "2026-07-27, unsettled but actionable: set_time is NOT reliably ack-bearing. P5 saw it "
        "reply twice per call with the schedule subsystem idle and then fall COMPLETELY SILENT, "
        "three calls running, once a schedule theme was armed (2.0s settle, so genuine silence). "
        "A caller that awaits a reply for set_time -- anything routing it through a response=True "
        "path -- can therefore hang on a device with an armed schedule theme. The SDK does not "
        "await one, and should not start. Scope and confirm probe: see common.ack_timing.",
    ),
    Capability(
    ...
    Capability(
        ...
        "chunked GIF upload all acked on every repeat. SCOPE, and a known exception: P14 ran with "
        "an IDLE schedule subsystem, and it did not test common.set_time at all. P5 "
        "(probes/probe_p5_schedule.py, 2026-07-27) saw set_time draw TWO acks per call before a "
        "schedule theme was armed and then ZERO acks, three times running, once one was -- with "
        "the same 2.0s settle, so that is genuine silence and not the 2026-07-26 read-too-early "
        "instrumentation bug. Read 'never silent' as scoped to P14's conditions; ARMED SCHEDULE "
        "STATE IS AN UNCHARACTERISED VARIABLE for ack behaviour generally, not just for set_time. "
        "Observed-with-consequence, not settled: the confirm probe is "
        "probes/probe_p19_g4_settime_acks.py (queued, docs/PROBE_PLAN.md P19). "
        "First-ack latency clustered by family: FLAT config/"
