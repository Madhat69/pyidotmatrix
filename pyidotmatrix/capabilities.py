...
_ENTRIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        "left in force. ONLY THE DIY FRAME IS VOLATILE ON THE BLE-RECONNECT COLUMN -- it resets "
        ...
        "normally (StatusAck status=3, SAVED), and it is silently gone at the next BLE reconnect "
        "or software power cycle -- the panel returns to the clock, with no error raised anywhere "
        "in the stack. ONE intervening BLE disconnect/reconnect within the same client session "
        "makes everything uploaded afterwards durable. EVIDENCE, every GIF-row run of the "
        "session, seven runs, perfect separation on exactly ONE column (did a BLE disconnect/"
        "reconnect happen earlier in this process?): `gif` x2 (reset, no preceding row, no "
        "reconnect) DIED; `--delay 120 gif` (reset, no preceding row, no reconnect) DIED; "
        "`--no-reset gif` x2 (NO reset, no preceding row, no reconnect) DIED; the full row sweep "
        "(reset, preceding row, reconnect) SURVIVED; `clock gif` (reset, preceding row, "
        "reconnect) SURVIVED; `--preamble ble gif` (reset, NO preceding row, RECONNECT) SURVIVED. "
        ...
        "120s of silence); and a PRECEDING ROW or the mode change it performs (`--preamble ble` "
        "had no preceding row and survived, so the rescue is the reconnect itself). Scope is "
        ...
        "it. SCOPE LIMIT, DO NOT OVER-GENERALISE: this is NOT 'nothing survives a first "
        ...
        "commit normally on a first connection. MECHANISM: OPEN. The probe's reconnect calls "
        ...
        "in our stack distinguishes the two connections; the mechanism is unexplained. THE ONE "
        "DISCRIMINATOR STILL UNRUN is `probe_p11_persistence.py --preamble power gif`, which "
        "does turn_off/turn_on over the SAME BLE connection with no disconnect: if it SURVIVES "
        "the shadow is device-side display state that any re-initialisation lifts; if it DIES "
        "the shadow is bound to the BLE SESSION specifically. That run decides which mitigation "
        "is even possible, so NO mitigation is implemented in this driver yet. GLANCEOS "
    ...
    Capability(
        ...
        "the SDK sent explaining the change. CAVEAT, CORRECTED 2026-07-27: this caveat used to "
        "attribute phase 9's loss to a candidate 'reset shadow', because phase 9 in probes/"
        "probe_p7_odds_and_ends.py runs immediately after that same probe's timer-state-machine "
        "phase, whose cleanup calls common.reset() shortly before the magenta color.show(). THE "
        "RESET IS NOT THE VARIABLE -- see display.persistence_matrix, where `--no-reset` died "
        "identically and the finding was renamed THE FIRST-CONNECTION SHADOW. What phase 9 "
        "shares with those runs is the thing that actually separates them: the magenta was "
        "pushed on the FIRST BLE connection of its process, with no intervening reconnect. It is "
        "NOT known whether phase 9's loss is genuine device-side colour volatility or an "
        "instance of the first-connection shadow; treat it as an open, genuine observation, not "
        "a solved mechanism. INDEPENDENT DATA POINT, same session (P11, see display."
        "persistence_matrix): the persistence matrix's fullscreen-colour row -- set after the "
        "run's earlier rows had already reconnected -- HELD across an automated BLE reconnect, "
        "which is evidence for persistence under the same interruption type but does NOT resolve "
        "which of the two explanations phase 9 is, since the runs differ in exactly that "
        "variable. A retest of phase 9's scenario after an intervening reconnect is queued in "
        "docs/PROBE_PLAN.md to separate the two.",
    ),
    Capability(
