...
_ENTRIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        "committed rather than lingering. MATCHED CONTROL, same probe's `no-arm` sequence "
        "2026-07-28: MAGENTA written with NO arming reconnect and the SAME 8 s hold BOOTED "
        "BLUE -- magenta never reached flash. So arm/no-arm differ in exactly one variable "
        "(the prior reconnect) and give opposite commit outcomes at identical hold times, "
        "which is the strongest evidence in this series. It also EXCLUDES the worry that the "
        "(A) ladder measured the wrong quantity: a reconnect can only read the ACTIVE MODE "
        "while a power cycle reads FLASH, so if the unassisted commit were fast and only "
        "display takeover slow, magenta would have booted. It did not -- the unassisted "
        "commit is genuinely slow. GLANCEOS: this EXPLAINS the daemon's double-tap "
