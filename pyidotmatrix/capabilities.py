...
_ENTRIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        "not held constant until the very last probe. THE MODEL: display-mode state -- which "
        ...
        "HAS PASSED SINCE THE WRITE. Dies at ~8 s across many runs, both GIF and fullscreen "
        "colour; SURVIVES at 90 s (probes/probe_p19_g5_kill_event.py `own-delayed`, the same "
        "session performing both the write and the reconnect, so session identity is held "
        "constant and only the delay varies). The threshold is between 8 s and 90 s and is NOT "
        "YET BISECTED; the 07-12 'under ~3 min' figure is consistent with it. (B) A PRIOR "
        ...
        "prior-reconnect rescue is a REAL, SEPARATE effect and ITS MECHANISM IS OPEN. EVIDENCE, "
        ...
        "sweep (prior reconnect supplied by the preceding row, ~8 s) SURVIVED; `g5 own-delayed` "
        "(fullscreen colour, no prior reconnect, 90 s dwell) SURVIVED; `g5 reconnect` (foreign "
        "session, minutes of dwell) SURVIVED; `set brightness` followed by a PHYSICAL mains power "
        "cut (minutes) SURVIVED, the panel booted to it. RULED OUT as the operative variable: "
        ...
        "prove NOTHING about session identity; `g5 own-delayed` then showed the SAME session "
        "failing to kill its own 90-second-old content. Session identity is not the variable and "
        ...
        "reconnect. CONSISTENT-WITH, from the older record: color.show's fullscreen colour "
        "survived THREE DAYS including power cycles (2026-07-17 persistence probes) -- pushed "
        "once and then left alone for days, i.e. condition (A) satisfied with room to spare. "
        ...
        "let content DWELL or to perform a real throwaway connect/disconnect/reconnect. For "
