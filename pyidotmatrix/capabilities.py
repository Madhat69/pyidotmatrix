...
_ENTRIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        "power-cycle exercised Timer alarms only; queued in docs/PROBE_PLAN.md. OPEN DEFECT "
        "SURFACE, UNEXPLAINED, now THREE INSTANCES: (1) a GIF uploaded immediately after "
        ...
        "SURVIVED, also reproduced twice. (2) color.show's P7 phase 9 result: a magenta "
        "fullscreen colour set shortly after that phase's own common.reset() cleanup did not "
        "survive a 12s disconnect/reconnect (see color.show; that result is a genuine "
        "observation, not void). (3) By contrast, this same P11 run's fullscreen-colour row -- "
        "set well clear of any reset -- HELD across a BLE reconnect. Whether the rescuing factor "
        "across all three is ELAPSED TIME since the reset or an INTERVENING RE-INITIALISATION "
        "(a reconnect/power-cycle the preceding row itself supplies) is UNRESOLVED; do not treat "
        "this as a solved mechanism. common.reset() is the driver's own go-to for clearing a "
        "stuck state and GlanceOS's recovery paths do exactly that, so a recovery sequence that "
        "resets, then pushes content, then meets any reconnect can silently fail to persist with "
        "no error raised anywhere. probes/probe_p11_persistence.py now supports repeated row "
        "keys (`gif gif`) and a `--delay N` settle knob to separate the two candidates; re-runs "
        "queued in docs/PROBE_PLAN.md.",
    ),
    Capability(
    ...
    Capability(
        ...
        "the SDK sent explaining the change. CAVEAT: phase 9 in probes/probe_p7_odds_and_ends.py "
        "runs immediately after that same probe's timer-state-machine phase, whose cleanup calls "
        "common.reset() shortly before the magenta color.show() -- see display."
        "persistence_matrix's 'reset shadow' open defect surface, which now has three unexplained "
        "instances (a GIF dying in reset's shadow twice, and this phase 9 result) where content "
        "set soon after common.reset() fails to persist through an interruption. It is NOT known "
        "whether phase 9's loss is genuine device-side colour volatility or another instance of "
        "the reset shadow; treat it as an open, genuine observation, not a solved mechanism. "
        "INDEPENDENT DATA POINT, same session (P11, see display.persistence_matrix): the "
        "persistence matrix's fullscreen-colour row -- set well clear of any reset -- HELD "
        "across an automated BLE reconnect, which is evidence for persistence under the same "
        "interruption type but does NOT resolve whether phase 9's loss was the reset shadow or "
        "genuine volatility, since the two runs differ in exactly that variable. A retest of "
        "phase 9's scenario WITHOUT a preceding common.reset() is queued in docs/PROBE_PLAN.md "
        "to separate the two.",
    ),
    Capability(
